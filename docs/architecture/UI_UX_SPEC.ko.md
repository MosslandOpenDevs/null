...
월드 생성 중 정적 로딩 스피너 대신 실시간 진행 화면을 표시합니다.

**구성 요소:**
- 회전하는 다이아몬드 + 맥동하는 중심점 (시각적 앵커)
- **프로그레스 바**: 백엔드 실시간 진행률 기반으로 채워짐 (world config JSONB의 `_genesis_progress`)
- **퍼센트 표시**: 바 옆에 숫자로 표시
- **단계 상세 텍스트**: 현재 작업 설명 (예: "Iron Syndicate 에이전트 소환 중...")
- **단계 카운터**: "STEP 3 / 8" 형식으로 현재/전체 표시
- **시드 프롬프트 미리보기**: 시드 프롬프트 처음 80자

**백엔드 연동:**
- POST `/api/worlds`는 `status: "generating"`으로 즉시 응답
- 프론트엔드가 월드 페이지로 리다이렉트 후 2초마다 `GET /api/worlds/{id}` 폴링
- 각 제네시스 단계(월드 설정 → 파벌 → 파벌별 에이전트 → 관계 → 태그)마다 `world.config._genesis_progress` 갱신
- 완료 시 `"ready"`로 전환되며 폴링 종료
- 실패 시 `"error"`로 전환되며 홈 링크 표시
...
```json
{
  ...
  "step_num": 3,
  "total_steps": 8,
  "detail": "Iron Syndicate 에이전트 소환 중...",
  "percent": 38
}
```
