...
> **2026-07 업데이트** — 실제 코드베이스에 맞춰 갱신했습니다. 아래 Phase 4a–4d가 기술하는
> Three.js *코스모그래프* UI는 구현되지 않았고, 실제 UI는 동등 기능(헤럴드, 커맨드 팔레트,
> 개입, 북마크, 내보내기)을 갖춘 **크로니클**(피드형 관측소)입니다. 코스모그래프는 컨셉으로
> 남아 있습니다(docs/architecture/UI_UX_SPEC.ko.md 참고). 단기 목표는 **v0.3 Grounding**:
> 위키 근거 링크, 결정적 리플레이, UI 통합, Playwright E2E입니다.
...
- [x] 프로젝트 철학과 비전 정의
- [x] 문서 체계 생성
- [x] 모노레포 구조 설정 (backend / frontend / shared)
- [x] 개발 환경 구성 (Docker, CI/CD)
- [x] 코딩 표준 및 린팅 설정
...
- [ ] 위키 근거 링크 — 모든 문서 주장에 출처 대화/주장 연결
- [ ] 결정적 리플레이 — 실행별 모델·프롬프트·파라미터·RNG 시드 보존
- [ ] UI 통합 — 고아 상태인 KnowledgeHub/IntelPanel 트리를 Chronicle로 흡수
- [ ] 핵심 뷰어 경로에 대한 Playwright E2E
- [ ] 이벤트 원장 — 랜덤/주입 이벤트를 일급 레코드로 영속화

## v1: Research Preview
*데모가 아닌 재현 가능한 실험실.*

- [ ] 시나리오 포맷 + 스크립트형 월드 구성 SDK
- [ ] 배치 비교 — 동일 시드를 여러 설정으로 실행해 결과 비교
- [ ] 능력·한계·"합성 서사이며 정책 예측이 아님"을 명시한 모델 카드
- [ ] SECURITY.md / CODEOWNERS / semver 릴리스
